...
magic-editor（magic-api 原生）与 MagicOps 发布链路是两条独立路径。双轨决策采用方向 A：

- magic-editor 仅用于在线调试，不在生产承担执行；
- 生产执行必须走发布链路：Console 审批 -> 签名 -> 推送 -> Runtime 验签激活 -> 脚本引用执行。
- Runtime 执行端点采用脚本引用模式，拒绝裸 SQL/裸目标调用（请求只携带 `scriptId`，执行内容来自已签名发布包）。

日常运营人员若在编辑器写脚本直接跑，治理链条不生效；生产执行必须经发布链路强制。
